...
- **tmux** — Standalone `tmux.conf` (`~/.config/tmux/`). Prefix is `~` (tilde), vim-style pane nav (`hjkl`), capital-letter directional splits (`HJKL`), `t` for new window, `1`–`9` to jump, `s` for tree picker, `~ ~` for last-window. Move-pane bindings: `m <1-9>` joins the current pane into that window, `m 0` breaks it into a new window at the end, `M` breaks it into a new window right after current. Theme uses ANSI palette slots (`colour0`..`colour15`) so it tracks Ghostty's light/dark auto-switch. Previous Oh My Tmux-based override is retained as `tmux.conf.local.bak`; the `omt/` submodule is still in the tree but unreferenced.
