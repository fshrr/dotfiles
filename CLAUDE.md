...
- **zed** — Zed editor config (`~/.config/zed/settings.json`). Not part of any install profile; stow manually. **Claude ACP model gotcha:** the registry Claude agent lives under `agent_servers.claude-acp` (package `@agentclientprotocol/claude-agent-acp`). Its `default_config_options` accepts `mode` and `effort` (Zed validates these against the agent's advertised option values), but **`model` must be an exact SDK id** — Zed strict-validates it and silently drops aliases like `opus` or `claude-opus-4-8`, logging `WARN [agent_servers::acp] \`…\` is not a valid value for config option \`model\`` to `~/Library/Logs/Zed/Zed.log`. To pin the model by alias, set `env.ANTHROPIC_MODEL` on the agent instead (highest-priority source, fuzzy-resolved so `opus`/`opus[1m]` work) — or select it from the in-thread model dropdown, which makes Zed write the exact id into `default_config_options.model`.
